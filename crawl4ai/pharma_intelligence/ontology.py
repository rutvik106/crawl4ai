...
# Higher = more strategic importance for leadership decision-making
...
ALL_CATEGORIES: List[str] = [
    ...
    "Competitive Intelligence",
    "Indian Market",
    # Priority tiers (assigned by pipeline, not by LLM)
    "Key Highlight",
    "Other News",
]


# ── Categories that qualify an article as "Key Highlight" ───────────────────
...
# Minimum relevance score (0-100) to be promoted to Key Highlight
KEY_HIGHLIGHT_SCORE_THRESHOLD = 60


# ── Regulatory bodies recognised by the system ─────────────────────────────
...
# ── Therapy areas for entity extraction ────────────────────────────────────
...
# Articles matching ANY of these are excluded BEFORE LLM processing.
# This reduces cost and ensures consistent filtering.
...
# ── Patterns that strongly signal inclusion ────────────────────────────────
...
def is_hard_excluded(text: str) -> bool:
    """Returns True if text matches any mandatory exclusion pattern."""
    combined = text[:2000]  # Only check first 2000 chars for speed
    return any(p.search(combined) for p in COMPILED_EXCLUSIONS)


def has_strong_include_signal(text: str) -> bool:
    """Returns True if text matches any strong-include pattern."""
    combined = text[:2000]
